...
### Bug Fix — 2026-02-23 : Validation credentials Meta manquante

**Bug** : `setWhatsAppConfig` sauvegardait n'importe quelles valeurs en DB sans vérifier leur validité auprès de l'API Meta. L'UI affichait "connecté" même avec des credentials incorrects.

**AC manquant (ajouté rétroactivement)** :
> **Given** je soumets des credentials WhatsApp (Phone Number ID + Access Token)
> **When** l'API Meta rejette ces credentials (401/400)
> **Then** une erreur `BAD_REQUEST` est retournée et rien n'est sauvegardé en DB

**Fix appliqué** dans `src/server/api/routers/settings.ts` — `setWhatsAppConfig` :
- Ajout d'un appel `fetch` à `https://graph.facebook.com/v20.0/{phoneId}?access_token={token}` avant toute persistance
- Si `!metaRes.ok` → `TRPCError({ code: "BAD_REQUEST" })`
- Si le réseau est injoignable → `TRPCError({ code: "INTERNAL_SERVER_ERROR" })`
- Si `metaAccessToken` est `null` (préservation du token existant) → pas de validation Meta (comportement inchangé)

**Tests ajoutés** dans `src/server/api/routers/settings.test.ts` :
- `rejects invalid Meta credentials with BAD_REQUEST`
- `rejects when Meta API is unreachable with INTERNAL_SERVER_ERROR`
- `skips Meta API validation when metaAccessToken is null`
- Tous les tests existants mis à jour avec mock `fetch` (`vi.stubGlobal`)
